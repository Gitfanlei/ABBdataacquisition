...
//  Call parent  processTopicLink
function processTopicLink() {
    //Add onclick event to all a tags.
    var aTags = document.getElementsByTagName("a");
    for (var x = 0; x < aTags.length; x++) {
        aTags[x].onclick = function () {
            parent.processTopicLink(this.href);
        };
    }
}
...
if (document.addEventListener)
    document.addEventListener("DOMContentLoaded", function () { alreadyrunflag = 1; processTopicLink() }, false)
//IE
...
//Safari
    ...
    var _timer = setInterval(function () {
        if (/loaded|complete/.test(document.readyState)) {
            ...
            processTopicLink() // call target function
        }
    }, 10)
}
//Others
window.onload = function () {
    setTimeout("if (!alreadyrunflag){processTopicLink()}callResize()", 0)
}
